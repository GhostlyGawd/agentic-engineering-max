...
- **A cross-compat fix lands with the same testing discipline as any bug fix** (regression test; see Testing above). `tests/test-crosscompat-lint.ps1` locks the lint's own behavior.