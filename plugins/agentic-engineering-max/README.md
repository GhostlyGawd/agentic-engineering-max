...
- **`/plugin marketplace add` fails with "Host key verification failed" / "SSH host key is not in your known_hosts file".** The clone went over SSH (`git@github.com`) and this machine has never accepted GitHub's host key. This is a one-time per-machine setup, not a problem with the repo. Fastest fix: run `ssh -T git@github.com` once and answer `yes` to accept the fingerprint (GitHub's published ED25519 fingerprint is `SHA256:+DiY3wvvV6TuJJhbpZisF/zLDA0zPMSvHdkr4UvCOqU` -- verify it matches before trusting), then re-run the install. If you never intended installs to use SSH at all, check for a global rewrite rule with `git config --global --get-regexp "url.*insteadof"`; a rule like `url.git@github.com:.insteadof https://github.com/` forces every clone onto SSH -- remove it with `git config --global --unset url."git@github.com:".insteadOf` and the install will clone over HTTPS, no host key needed.
